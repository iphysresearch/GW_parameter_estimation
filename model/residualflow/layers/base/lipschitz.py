...
from .mixed_lipschitz import InducedNormLinear, InducedNormConv2d, InducedNormConv1d

__all__ = ['SpectralNormLinear', 'SpectralNormConv2d', 'LopLinear', 'LopConv2d', 'get_linear', 'get_conv2d'] + \
          ['LopConv1d', 'get_conv1d']
    ...
    def _initialize_u_v(self):
        if self.kernel_size == (1, 1):
            ...
            with torch.no_grad():
                ...
                v = F.normalize(torch.randn(num_input_dim).to(self.weight), dim=0, eps=1e-12)
                # forward call to infer the shape
                u = F.conv2d(v.view(1, c, w), self.weight, stride=self.stride, padding=self.padding, bias=None)
...
class LopConv1d(nn.Conv1d):
    ...
    def __init__(
        ...
        **unused_kwargs,
        ...
        del unused_kwargs
        super(LopConv1d, self).__init__(in_channels, out_channels, kernel_size, stride, padding, bias)
        self.coeff = coeff
        self.domain = domain
        self.codomain = codomain
        self.local_constraint = local_constraint
    ...
    def extra_repr(self):
        s = super(LopConv1d, self).extra_repr()
        return s + ', coeff={}, domain={}, codomain={}, local={}'.format(
            self.coeff, self.domain, self.codomain, self.local_constraint
        )
...
def get_conv1d(
    ...
    if domain == 1:
        ...
        if domain in [2, float('inf')]:
            _conv1d = LopConv1d
    return _conv1d(in_channels, out_channels, kernel_size, stride, padding, bias, coeff, domain, codomain, **kwargs)
