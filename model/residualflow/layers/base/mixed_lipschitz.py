try:
    ...
    print('Using "import collections.abc as container_abcs" in base/mixed_lipschitz.py.')
...
__all__ = ['InducedNormLinear', 'InducedNormConv2d'] + ['InducedNormConv1d']
...
class InducedNormConv1d(nn.Module):
    ...
    def __init__(
        ...
        del unused_kwargs
        super(InducedNormConv1d, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        ...
        with torch.no_grad():
            ...
            if self.kernel_size == (1, 1):
                ...
                with torch.no_grad():
                    ...
                    # forward call to infer the shape
                    u = F.conv1d(
                        self.v.view(1, c, w), self.weight, stride=self.stride, padding=self.padding, bias=None
                    )
