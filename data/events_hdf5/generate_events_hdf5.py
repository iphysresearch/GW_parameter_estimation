...
for event in tqdm(event_list):
    ...
    try:
        i = 1
        j = 0
        h1_inject = TimeSeries.fetch_open_data('H1',
                                               t_event + T_buffer - T - i*T_psd,
                                               t_event + T_buffer + j*T_psd, cache=True)
        print('H1:', len(h1_inject))
        assert False == (True in np.isnan(h1_inject.value))
        l1_inject = TimeSeries.fetch_open_data('L1',
                                               t_event + T_buffer - T - i*T_psd,
                                               t_event + T_buffer + j*T_psd, cache=True)
        ...
        if len(detectors) == 3:
            v1_inject = TimeSeries.fetch_open_data('V1',
                                                   t_event + T_buffer - T - i*T_psd,
                                                   t_event + T_buffer + j*T_psd, cache=True)
