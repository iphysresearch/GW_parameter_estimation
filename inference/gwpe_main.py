...
class PosteriorModel(object):
    ...
    def load_dataset(self, batch_size=512, detectors=None,
                     ...
                     sampling_from=None,
                     nsample=10000, nsamples_target_event=0,
                     mixed_alpha=None,
                     bw_dstar=None):
        ...
        self.wfd.load_setting(self.data_dir,
        ...
        if self.wfd.sampling_from == 'posterior':
            ...
            self.wfd._cache_oversampled_parameters(
            ...
            parameters_posterior = self.wfd._sample_prior_posterior(nsample)\
                                           .astype(np.float32)
            parameters_uniform = self.wfd._sample_prior(nsample)\
                                         .astype(np.float32)
            self.wfd.parameters = np.concatenate((
                parameters_posterior[:int(nsample*mixed_alpha)],
                parameters_uniform[:(nsample-int(nsample*mixed_alpha))]),
                axis=0)
            self.wfd.nsamples = len(self.wfd.parameters)
    ...
    def train(self, epochs,
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            if self.model_dir is not None:
                ...
                if epoch == 1:
                    with open(p / (('a{}_'.format(self.wfd.mixed_alpha)
                              if self.wfd.mixed_alpha else '')+'history.txt'),
                              'w') as f:
                    ...
                    if self.model_type == 'cvae':
                        with open(p / 'kl_history.txt', 'a') as f:
                            ...
                            writer.writerow(
                                [epoch, train_kl_loss, test_kl_loss])
                    data_history = np.loadtxt(p / (('a{}_'.format(
                                              self.wfd.mixed_alpha)
                    ...
                    self._extracted_from_plot_kljs_history_121('Loss', p,
                                                               'history.png')
                    epoch_minimum_test_loss = int(data_history[np.argmin(
                                                  data_history[:, 2]), 0])
                ...
                if ((output_freq is not None) and
                   (epoch == epoch_minimum_test_loss)):
                    for f in os.listdir(p):
                        if (
                            '_model.pt' in f
                            or '_waveforms_supplementary.hdf5' in f
                        ):
                            os.remove(p / f)
                    print('Saving model as e{}_{} & e{}_{}'.format(
                        epoch, self.save_model_name, epoch,
                        self.save_aux_filename)
                    )
                    self.save_model(filename='e{}_'.format(epoch)
                                             + self.save_model_name,
                                    aux_filename='e{}_'.format(epoch)
                                                 + self.save_aux_filename)
    ...
    def save_kljs_history(self, p, epoch, event):
        ...
        if epoch == 1:
            with open(p / (('a{}_'.format(self.wfd.mixed_alpha)
                      if self.wfd.mixed_alpha else '')
                      + 'js_{}_history.txt'.format(event)), 'w') as f:
                writer = csv.writer(f, delimiter='\t')
                writer.writerow(list(self.wfd.param_idx.keys()))
            with open(p / (('a{}_'.format(self.wfd.mixed_alpha)
                      if self.wfd.mixed_alpha else '')
                      + 'kl_{}_history.txt'.format(event)), 'w') as f:
                writer = csv.writer(f, delimiter='\t')
                writer.writerow(list(self.wfd.param_idx.keys()))

        with open(p / (('a{}_'.format(self.wfd.mixed_alpha)
                  if self.wfd.mixed_alpha else '')
                  + 'js_{}_history.txt'.format(event)), 'a') as f:
            writer = csv.writer(f, delimiter='\t')
            writer.writerow(js_divergence(
                [self.test_samples[:, index],
                 self.all_bilby_samples[event][:, index]])
                for name, index in self.wfd.param_idx.items())
        with open(p / (('a{}_'.format(self.wfd.mixed_alpha)
                  if self.wfd.mixed_alpha else '')
                  + 'kl_{}_history.txt'.format(event)), 'a') as f:
            writer = csv.writer(f, delimiter='\t')
    ...
    def plot_kljs_history(self, p, epoch):
        ...
        for event in tqdm(['GW150914', 'GW151012',
            ...
            jsdf = pd.read_csv(p / (('a{}_'.format(self.wfd.mixed_alpha)
                               if self.wfd.mixed_alpha else '')
                               + 'js_{}_history.txt'.format(event)), sep='\t')
            plt.fill_between(range(len(jsdf)),
                             jsdf.mean(axis=1),
                             jsdf.max(axis=1), alpha=0.6, label=event)
        self._extracted_from_plot_kljs_history_121('JS div.', p,
                                                   'js_history.png')

    def _extracted_from_plot_kljs_history_121(self, arg0, p, arg2):
        plt.xlabel('Epoch')
        plt.ylabel(arg0)
        plt.legend()
        plt.savefig(
            (
                ...
                / (
                    (
                        (
                            'a{}_'.format(self.wfd.mixed_alpha)
                            if self.wfd.mixed_alpha
                            else ''
                        )
                        + arg2
                    )
...
def parse_args():
    ...
    dir_parent_parser.add_argument('--no_cuda', action='store_false',
                                   dest='cuda')
    dir_parent_parser.add_argument('--dont_sample_extrinsic_only',
                                   action='store_false')
...
def main():
    ...
    if args.mode == 'train':
        ...
        if args.transfer_epochs:
            print('Now, transfer learning from alpha={}!'
                  .format(args.mixed_alpha))
            try:
                ...
                if args.save:
                    print('Saving model')
                    pm.save_model(filename='a{}'.format(args.mixed_alpha)
                                           + pm.save_model_name,
                                  aux_filename='a{}'.format(args.mixed_alpha)
                                           + pm.save_aux_filename)
            alpha_list = [0.8, 0.6, 0.4, 0.2, 0.1, 0.05, 0.02, 0.01, 0.005]
            for i, mixed_alpha in enumerate(alpha_list):
                print('Transfer learning by starting with alpha={}!'
                      .format(mixed_alpha))
                pm.load_dataset(batch_size=args.batch_size,
                                detectors=args.detectors,
                                truncate_basis=args.truncate_basis,
                                ...
                                distance_prior_fn=args.distance_prior_fn,
                                sampling_from=args.sampling_from,
                                nsamples_target_event=args
                                .nsamples_target_event,
                ...
                try:
                    ...
                    if args.save:
                        print('Saving model')
                        pm.save_model(filename='a{}'.format(mixed_alpha)
                                               + pm.save_model_name,
                                      aux_filename='a{}'.format(mixed_alpha)
                                                   + pm.save_aux_filename)
